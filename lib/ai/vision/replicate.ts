/**
 * Generic Replicate prediction client — shared by every Replicate-hosted
 * model this app calls (segmentation in lib/ai/vision/segmentation.ts,
 * FLUX Fill compositing in lib/ai/fluxFill.ts). A new provider beyond the
 ...
 * app. Model choice is deliberately an env var per caller, not a hardcoded
 * version hash — Replicate models get deprecated/renamed; verify the exact
 ...
 *
 * Restored from git history (commit 59d9d2a) where this was first built
 * for a since-reverted segmentation attempt — the generic client itself
 * was sound and needed no changes; only the model-specific callers (which
 * assumed an unverified schema) were rewritten.
 */
